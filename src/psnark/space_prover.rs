...
impl<E: PairingEngine> Proof<E> {
        ...
        SG::Item: Borrow<E::G1Affine>,
    {
        ...
        let (
            (pl_set_sh_a, pl_set_acc_a),
            (pl_subset_sh_a, pl_subset_acc_a),
            (pl_sorted_sh_a, pl_sorted_acc_a),
        ) = plookup_streams(r_a, r_a_star, row_a, y, z);

        let (
            (pl_set_sh_b, pl_set_acc_b),
            (pl_subset_sh_b, pl_subset_acc_b),
            (pl_sorted_sh_b, pl_sorted_acc_b),
        ) = plookup_streams(r_b, r_b_star, row_b, y, z);

        let (
            (pl_set_sh_c, pl_set_acc_c),
            (pl_subset_sh_c, pl_subset_acc_c),
