...
pub(crate) fn random_vector<F>(size: usize, rng: &mut impl RngCore) -> Vec<F>
where
    F: Field,
{
    (0..size).map(|_| F::rand(rng)).collect::<Vec<_>>()
}

/// Evaluation of a multilinear polynomial at point `x`.
/// Consider the i'th coefficient of n-dimensional polynomial, and i's binary representation
/// $i_0, i_1 \cdots i_n$. Multiply the i'th coefficient by $\prod_{j=1}^{n}i_jx_j$
...
///
pub fn multi_poly_decompose<F: Field>(polynomial: &[F], eval_point: &[F]) -> (Vec<Vec<F>>, F) {
