...
impl<E: Pairing> Add for EvaluationProof<E> {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        EvaluationProof((self.0.into_group() + rhs.0.into_group()).into_affine())
    }
}

impl<E: Pairing> core::iter::Sum for EvaluationProof<E> {
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        let zero = EvaluationProof(E::G1Affine::identity());
...
impl<E: Pairing> VerifierKey<E> {
    ...
    pub fn verify(
        ...
        let lhs =
            commitment.0.into_group() - self.powers_of_g[0].mul_bigint(evaluation.into_bigint());
